...
<h4>Total lost revenue:I recommend the business prioritize investigating the root cause of the delayed or lost shipments in those specific months to determine the root cause and determine if it was a route, client or driver issue.</h4>

<h4>Average delay in days:its the same as the above for finding the root cause for the extra delay in shipments being delivered. And discovering which routes and warehouses are responsible for the extra delays. Start by auditing Drivers assignments, road-type/traffic level, and vehicle condition.</h4>
